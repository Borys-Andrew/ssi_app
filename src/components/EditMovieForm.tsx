...
const EditMovieForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const movieToEdit = db.movies.find((movie: Movie) => movie.id === id);
    const { title, description, poster } = movieToEdit;
  ...
  const handleSubmit = () => {
    ...
    const db = JSON.parse(localStorage.getItem('db_app') as string);
    const movieIndex = db.movies.findIndex((movie: Movie) => movie.id === id);
